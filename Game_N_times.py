...
def main():
    ...
    # RUN STATS A LOT OF TIMES UNCOMMENT THIS OUT AND COMMENT ABOVE
    playerAI = PlayerAI()  # change this to PlayerAI() to test your player!
    computerAI = MediumAI()  # change this to a more sophisticated player you've coded
    displayer = Displayer()
    counter = 0
    ...
    n = 100
    for i in range(n):
        game = GAME_N_TIMES(playerAI=playerAI, computerAI=computerAI, N=7, displayer=displayer)
        if game.play() == 1:
            counter += 1  
        else: 
            counter_l += 1
        
        print(i, "cur win:" +str(counter)+" cur loss:"+str(counter_l))


    print("win rate with "+str(n)+" games: "+  str(float(counter/n)*100) + "%")
